...
// extern int NUM_SAMPLES; // Number of rays to sample per area light
// extern struct colourRGB BACKGROUND; // Background colour for scene
...
/* The structure below defines an RGB colour, values are in [0,1] */
struct colourRGB{
   double R;
   double G;
   double B;
...
struct ray3D{
   ...
   void (*rayPos)(struct ray3D *ray, double lambda, struct point3D *pos);
               ...
               // for a given lambda.
   struct colourRGB col;   // Keeps track of the ray's colour as it bounces through the scene
   struct colourRGB I;     // Accumulators of brightness for explicit light sampling
   struct point3D srcN; // Source normal - needed for diffuse illumination

   // To keep track of refraction indices the ray has travelled through
   double r_index[10]; // no more than 10 nested objects supported.
   int r_index_index;
};
...
struct object3D{
   ...
   // for triangles: stores vertices. different than a3's hacky way of doing this.
   struct point3D v1;
   struct point3D v2;
   struct point3D v3;
   ...
   struct image *normalImg;         // Normal map for this object
   struct image *alphaImg;          // Alpha map for the object

   struct Box box; // box containing object for RT acceleration
   ...
   struct object3D *LSnext;  // For light sources, pointer to next.
...
struct view{
   ...
   double focus_distance; // Focus distance
